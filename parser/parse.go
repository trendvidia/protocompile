...
// parsing succeeded without errors; warnings and remarks do not make it fail.
//
...
func Parse(path string, source *source.File, r *report.Report) (file *ast.File, ok bool) {
	...
	for _, d := range r.Diagnostics[prior:] {
		// [report.Level] runs most-severe-first — ICE(1), Error(2),
		// Warning(3), Remark(4) — so error-or-worse is `<= report.Error`.
		// The other direction reports failure for a file whose only
		// diagnostic is a warning, and success for one that hit an ICE.
