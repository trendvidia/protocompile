...
// lowering succeeded without errors; warnings and remarks do not make it fail.
//
...
func (s *Session) Lower(source *ast.File, errs *report.Report, importer Importer) (file *File, ok bool) {
	...
	for _, d := range errs.Diagnostics[prior:] {
		...
		// `<= report.Error`. The other direction reports failure for a
		// file whose only diagnostic is a warning (an unused import, say)
		// and success for one whose lowering hit a recovered ICE.
