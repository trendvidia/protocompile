...
type Executor struct {
	...
	// spawnBudget caps the number of in-flight async sub-task goroutines
	// the executor will create. When this is saturated, [task.start] runs
	// the sub-task synchronously in the caller's goroutine instead of
	// spawning a fresh one. Without this cap, deeply-nested fan-outs (like
	// the googleapis transitive-import graph) easily spawn 10k+
	// goroutines all parked in [Task.acquire]. The work is correctly
	// bounded by [Executor.sema] either way, but the unbounded goroutine
	// count creates scheduler thrash and amplifies race-detector overhead
	// on CI runners.
	//
	// Default capacity is 4*GOMAXPROCS — enough headroom that wide
	// fan-outs at the executor root still spawn goroutines (preserving
	// parallelism), while capping the worst case so a 10k-import graph
	// can't blow up the goroutine pool. Tune via [WithGoroutineBudget].
	spawnBudget chan struct{}
...
func New(options ...ExecutorOption) *Executor {
	...
	exec := &Executor{
		sema:        semaphore.NewWeighted(int64(procs)),
		spawnBudget: make(chan struct{}, 4*procs),
	}
...
// WithGoroutineBudget sets the maximum number of in-flight async sub-task
// goroutines the executor will spawn at once. Excess sub-tasks fall back to
// running synchronously in the caller's goroutine. Defaults to 4*GOMAXPROCS
// if not set explicitly. The minimum useful value is 1 (passing 0 or
// negative disables async spawning entirely).
func WithGoroutineBudget(n int) ExecutorOption {
	return func(e *Executor) {
		if n < 0 {
			n = 0
		}
		e.spawnBudget = make(chan struct{}, n)
	}
}
